...
class Vae(object):
    def __init__(self,gamma=100.0,capacity_limit=25.0,capacity_change_duration=100000,learning_rate=5e-4):
        self.gamma = gamma
    ...
    def _conv2d_weight_variable(self, weight_shape, name, deconv=False):
        Nm_weight = "W_{0}".format(name)
        Nm_bias = "b_{0}".format(name)
        ...
        weight = tf.get_variable(Nm_weight, weight_shape,
                             initializer=conv_initializer(w, h, input_channels))
        bias   = tf.get_variable(Nm_bias, bias_shape,
                             initializer=conv_initializer(w, h, input_channels))
        return weight, bias
    ...
    def _fc_weight_variable(self, weight_shape, name):
        ...
        weight = tf.get_variable(Nm_weight, weight_shape, initializer=fc_initializer(input_channels))
        bias   = tf.get_variable(Nm_bias, bias_shape,   initializer=fc_initializer(input_channels))
        return weight, bias
