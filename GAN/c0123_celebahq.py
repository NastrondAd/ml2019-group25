...
imgs = glob.glob("/home/2000G/xs/Datasets/celeba-HQ/celeba-64/*.jpg")
...
def gindex(feature, index, min, max):
    ...
    for i in range(10):
        zimgs[i] = zindex
        zimgs[i][z_dim + feature] = c1_lin[i]
...
allimgs = np.zeros((4, 10, img_dim, img_dim, img_channel))
# allimgs[0] = gindex(zc_sec, 0, 1, 10)
# allimgs[1] = gindex(zc_sec, 8, -5, 5)
small = -3
large = 3
allimgs[0] = gindex(0, 7 * 50 + 33, -3, 3)
allimgs[1] = gindex(1, 7 * 50 + 33, -7, 0)
allimgs[2] = gindex(2, 7 * 50 + 33, 0, 10)
allimgs[3] = gindex(3, 7 * 50 + 33, -5, 5)
...
jizhongimg = np.zeros((4 * img_dim, 10 * img_dim, 3))
